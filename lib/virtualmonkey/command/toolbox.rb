module VirtualMonkey
  module Command
    ...
    #
    (@@command_flags ||= {}).merge!("generate_ssh_keys" => [:yes])
    ...
    #
    (@@command_flags ||= {}).merge!("destroy_ssh_keys" => [:yes])
    ...
    #
    (@@command_flags ||= {}).merge!("populate_security_groups" => [:yes])
    ...
    #
    (@@command_flags ||= {}).merge!("populate_datacenters" => [:yes])
    ...
    #
    (@@command_flags ||= {}).merge!("populate_all_cloud_vars" => [:yes])
    ...
    #
    (@@command_flags ||= {}).merge!("api_check" => [:yes])
    ...
    def self.api_check(*args)
      ...
      if [0.1, 1.0, 1.5].include?(@@options[:api_version])
        ...
        warn "Invalid version number: #{@@options[:api_version]}".red
        exit(1)
      end
    end
  end
